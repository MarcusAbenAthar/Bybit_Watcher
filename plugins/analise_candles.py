...
class AnaliseCandles(Plugin):
    ...
    def executar(self, *args, **kwargs) -> bool:
        ...
        try:
            ...
            if resultados:
                kwargs["resultados_candles"] = resultados
            else:
                kwargs["resultados_candles"] = {
                    "direcao": "NEUTRO",
                    "forca": "FRACA",
    ...
    def _calcular_tamanho_relativo_e_multiplicador(self, candle, padrao):
        """Calcula o tamanho relativo do candle e o multiplicador."""
        if not candle or len(candle) < 4:
            return 0, 1.0
        abertura, fechamento, maximo, minimo = candle
    ...
    def calcular_confianca(self, dados):
        ...
        try:
            if dados is None or len(dados) < 3:
                return 0.0
            dados_np = np.array(dados, dtype=np.float64)
            volatilidade = np.std(dados_np[:, 4])
    ...
    def _processar_sinais(self, dados, resultados, symbol, timeframe):
        ...
        sinais = []
        for candle in dados:
            padrao_candle = self.identificar_padrao(
                dados
            )  # Passa todos os dados, não só o candle
            if padrao_candle:
                classificacao = self.classificar_candle(candle)
                sinal = self.gerar_sinal(
                    dados, symbol, timeframe
                )  # Usa dados completos
                if sinal and sinal["sinal"]:
                    sinal["classificacao"] = classificacao
                    sinais.append(sinal)
        return sinais
