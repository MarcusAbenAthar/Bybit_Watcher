...
class TestMediasMoveis(unittest.TestCase):
    ...
    def test_calcular_media_movel_simples(self, mock_talib):
        ...
        mock_talib.SMA = MagicMock(return_value=np.array([1, 2, 3]))

        # Chama a função calcular_media_movel com tipo "simples"
        resultado = plugin.calcular_media_movel([1, 2, 3, 4, 5], 3, tipo="simples")

        # Verifica se a função SMA do talib foi chamada com os argumentos corretos
        mock_talib.SMA.assert_called_once_with([1, 2, 3, 4, 5], timeperiod=3)

        # Verifica se o resultado é o esperado
        self.assertTrue(np.array_equal(resultado, np.array([1, 2, 3])))
    ...
    def test_calcular_media_movel_exponencial(self, mock_talib):
        ...
        mock_talib.EMA = MagicMock(return_value=np.array([2, 3, 4]))

        # Chama a função calcular_media_movel com tipo "exponencial"
        resultado = plugin.calcular_media_movel([1, 2, 3, 4, 5], 3, tipo="exponencial")

        # Verifica se a função EMA do talib foi chamada com os argumentos corretos
        mock_talib.EMA.assert_called_once_with([1, 2, 3, 4, 5], timeperiod=3)

        # Verifica se o resultado é o esperado
        self.assertTrue(np.array_equal(resultado, np.array([2, 3, 4])))
    ...
    def test_calcular_media_movel_ponderada(self, mock_talib):
        ...
        mock_talib.WMA = MagicMock(return_value=np.array([3, 4, 5]))

        # Chama a função calcular_media_movel com tipo "ponderada"
        resultado = plugin.calcular_media_movel([1, 2, 3, 4, 5], 3, tipo="ponderada")

        # Verifica se a função WMA do talib foi chamada com os argumentos corretos
        mock_talib.WMA.assert_called_once_with([1, 2, 3, 4, 5], timeperiod=3)

        # Verifica se o resultado é o esperado
        self.assertTrue(np.array_equal(resultado, np.array([3, 4, 5])))
