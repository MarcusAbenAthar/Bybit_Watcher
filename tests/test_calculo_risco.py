...
def klines_validos():
    # Gera 60 klines OHLCV fake (timestamp, open, high, low, close, volume)
    base = 1710000000000
    klines = []
    for i in range(60):
        ts = base + i * 60000
...
def test_inicializacao(plugin):
    ...
    assert plugin.PLUGIN_NAME == "calculo_risco"
...
def test_executar_com_dados_validos(plugin):
    ...
    assert "calculo_risco" in resultado
    assert isinstance(resultado["calculo_risco"], dict)
...
def test_executar_com_dados_faltantes(plugin):
    ...
    resultado = plugin.executar(dados_completos=dados, symbol="BTCUSDT", timeframe="1h")
    assert resultado["calculo_risco"]["direcao"] == "LATERAL"
    # Faltando symbol
...
def test_validar_klines(plugin):
    ...
    # Menos de 50 klines
    assert plugin._validar_klines(klines[:10], "BTCUSDT", "1h") is False
